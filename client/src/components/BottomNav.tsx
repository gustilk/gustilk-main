...
export default function BottomNav() {
  ...
  return (
    <nav
      ...
      style={{ background: "rgba(13,6,24,0.97)", backdropFilter: "blur(20px)", borderTop: "1px solid rgba(244,196,48,0.15)", paddingBottom: "env(safe-area-inset-bottom)" }}
    >
      {NAV_ITEMS.map(({ href, icon: Icon, tKey, id }) => {
        ...
        return (
          <Link
            ...
            style={{ color: isActive ? "#F4C430" : "rgba(51,51,51,0.3)" }}
            data-testid={`nav-${id}`}
          >
            <div className="relative">
              ...
              {isActivity && likesCount > 0 && (
                <span
                  className="absolute -top-1 -right-1 w-4 h-4 rounded-full flex items-center justify-center text-[9px] font-bold"
                  style={{ background: "#F4C430", color: "#F9F9F9" }}
