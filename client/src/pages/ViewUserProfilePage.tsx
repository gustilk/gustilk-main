...
export default function ViewUserProfilePage({ viewer, userId }: Props) {
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center h-screen" style={{ background: "#0d0002" }}>
  ...
  if (!profile) {
    return (
      <div className="flex flex-col items-center justify-center h-screen gap-4" style={{ background: "#0d0002" }}>
  ...
  return (
    <div className="flex flex-col min-h-screen pb-24" style={{ background: "#0d0002" }}>
          ...
          <div className="absolute top-0 left-0 right-0 flex gap-1 px-3 pt-14">
            {allPhotos.map((_, i) => (
              <button key={i} onClick={() => setPhotoIdx(i)}
                ...
                style={{ background: i === photoIdx ? "rgba(200,0,14,0.9)" : "rgba(255,255,255,0.3)" }} />
            ))}
          </div>
        )}
        ...
        {!isPremium && (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 pt-16">
            <div className="w-14 h-14 rounded-full flex items-center justify-center"
              style={{ background: "rgba(13,6,24,0.85)", border: "2px solid rgba(200,0,14,0.5)" }}>
              <Lock size={22} color="#c8000e" />
            </div>
      ...
      <div className="px-5 pt-4 space-y-4">
        ...
        <div className="flex items-start justify-between gap-3">
          ...
          {profile.isPremium && (
            <div className="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0"
              style={{ background: "rgba(200,0,14,0.1)", border: "1.5px solid rgba(200,0,14,0.3)" }}>
              <Star size={18} fill="#c8000e" color="#c8000e" />
            </div>
          )}
        </div>
        ...
        {showLikeActions && !actedOnLike && (
          <div className="flex gap-3">
            <button
              ...
              style={{ background: "rgba(224,48,80,0.12)", border: "1.5px solid rgba(224,48,80,0.45)", color: "#e03050" }}
            >
            ...
            <button
              ...
              className="flex-1 flex items-center justify-center gap-2 py-4 rounded-2xl font-bold text-sm transition-all active:scale-95 disabled:opacity-50"
              style={{ background: "rgba(200,0,14,0.12)", border: "1.5px solid rgba(200,0,14,0.5)", color: "#c8000e" }}
            >
        ...
        {/* ── Divider ── */}
        <div style={{ height: 1, background: "rgba(200,0,14,0.08)" }} />
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <button
            ...
            style={isPremium && match
              ? { background: "linear-gradient(135deg, #9b0010, #e03050)", color: "white", boxShadow: "0 4px 16px rgba(123,63,160,0.3)" }
              : isPremium && !match
                ? { background: "rgba(255,255,255,0.06)", color: "rgba(253,248,240,0.4)", border: "1px solid rgba(255,255,255,0.1)", cursor: "default" }
                : { background: "rgba(200,0,14,0.08)", color: "#c8000e", border: "1.5px solid rgba(200,0,14,0.3)" }
            }
          >
          ...
          <button
            ...
            style={isPremium && match
              ? { background: "rgba(200,0,14,0.1)", color: "#c8000e", border: "1.5px solid rgba(200,0,14,0.3)" }
              : !isPremium
                ? { background: "rgba(200,0,14,0.08)", color: "#c8000e", border: "1.5px solid rgba(200,0,14,0.3)" }
        ...
        {!isPremium && (
          <button
            ...
            style={{ background: "linear-gradient(135deg, #c8000e, #e83535)", color: "#1a0005", boxShadow: "0 6px 20px rgba(200,0,14,0.25)" }}
          >
            <Crown size={16} />
            ...
            <div className="flex gap-2 overflow-x-auto pb-1">
              {allPhotos.map((p, i) => (
                <button key={i} onClick={() => setPhotoIdx(i)} data-testid={`thumb-photo-${i}`}
                  ...
                  style={{ border: i === photoIdx ? "2px solid #c8000e" : "2px solid transparent", opacity: i === photoIdx ? 1 : 0.65 }}>
