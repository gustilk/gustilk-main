...
export default function PremiumPage({ user }: Props) {
  ...
  if (user.isPremium) {
    return (
      <div className="flex flex-col min-h-screen items-center justify-center pb-24 px-6 text-center" style={{ background: "#E30613" }}>
        <div className="w-24 h-24 rounded-full flex items-center justify-center mb-6" style={{ border: "3px solid #FFD700" }}>
  ...
  return (
    <div className="flex flex-col min-h-screen pb-24" style={{ background: "#E30613" }}>
      ...
      <div className="px-5 py-6 space-y-6">
          ...
          <div className="flex justify-center mb-4">
            {isFree ? (
              <div className="w-20 h-20 rounded-full flex items-center justify-center text-4xl"
                style={{ background: "rgba(255,215,0,0.08)", border: "2px solid #FFD700" }}>ðŸŽ‰</div>
            ) : (
        ...
        {/* Benefits */}
        <div className="rounded-2xl p-5 space-y-3" style={{ background: "rgba(255,255,255,0.10)", border: "1px solid rgba(255,215,0,0.15)" }}>
          {BENEFITS.map(({ icon: Icon, text }) => (
            <div key={text} className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0" style={{ background: "rgba(255,215,0,0.12)" }}>
                <Icon size={15} color="#FFD700" />
              </div>
        ...
        {!isFree && (
          <div className="space-y-4">
              ...
              {config.methods.map(id => {
                ...
                return (
                  <button
                    ...
                    style={isSelected
                      ? { background: "rgba(255,215,0,0.18)", border: "1.5px solid #FFD700", color: "#FFD700" }
                      : { background: "rgba(255,255,255,0.12)", border: "1.5px solid rgba(255,215,0,0.15)", color: "rgba(255,255,255,0.5)" }
                    }
                  >
                    <span className="flex items-center justify-center" style={{ color: isSelected ? "#FFD700" : "rgba(255,255,255,0.6)" }}>
                      {m?.icon}
                    </span>
            ...
            {method && (
              <div className="rounded-2xl p-4 space-y-3" style={{ background: "rgba(255,255,255,0.10)", border: "1px solid rgba(255,215,0,0.15)" }}>
                {method.redirectOnly ? (
        ...
        <button
          ...
          style={isFree
            ...
            : { background: "linear-gradient(135deg, #FFD700, #CC9900)", color: "#500004", boxShadow: "0 8px 24px rgba(255,215,0,0.4)" }
          }
        >
          <Star size={17} fill={isFree ? "white" : "#500004"} />
