    ...
    <div className="p-4 md:p-6 max-w-3xl mx-auto">
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center h-32 gap-2">
          <Megaphone size={32} color="rgba(244,196,48,0.3)" />
        ...
        <div className="space-y-3">
          {items.map(item => (
            <div key={item.id} data-testid={`announcement-${item.id}`}
              ...
              style={{ background: "rgba(0,0,0,0.04)", border: `1px solid ${item.active ? "rgba(244,196,48,0.2)" : "rgba(255,255,255,0.07)"}`, opacity: item.active ? 1 : 0.6 }}>
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex items-center gap-1.5">
                  <button onClick={() => toggleMutation.mutate({ id: item.id, active: !item.active })}
                    ...
                    style={{ background: "rgba(0,0,0,0.05)", color: item.active ? "#10b981" : "#6b7280" }}>
