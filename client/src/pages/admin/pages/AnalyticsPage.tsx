  ...
  return (
    <div className="p-4 md:p-6 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-5">
        {/* User breakdown */}
        <div className="rounded-2xl p-4" style={{ background: "rgba(0,0,0,0.04)", border: "1px solid rgba(244,196,48,0.1)" }}>
          ...
          <StatRow label="Premium" value={u.premium} color="#F4C430" />
        ...
        {/* Engagement */}
        <div className="rounded-2xl p-4" style={{ background: "rgba(0,0,0,0.04)", border: "1px solid rgba(244,196,48,0.1)" }}>
          <h3 className="text-cream text-sm font-semibold mb-3 flex items-center gap-2"><Heart size={14} color="#6BBF59" /> Engagement</h3>
          <StatRow label="Total Matches" value={e.totalMatches} color="#6BBF59" />
        ...
        <div className="rounded-2xl p-4" style={{ background: "rgba(0,0,0,0.04)", border: "1px solid rgba(244,196,48,0.1)" }}>
          ...
          <div>
            ...
            <Bar value={u.female} max={u.total} color="#6BBF59" />
          </div>
        </div>
        ...
        <div className="rounded-2xl p-4" style={{ background: "rgba(0,0,0,0.04)", border: "1px solid rgba(244,196,48,0.1)" }}>
          ...
          {[
            { label: "Sheikh", value: u.sheikh, color: "#F4C430" },
            ...
            {dailySignups.map((d, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-1" title={`${d.day}: ${d.count}`}>
                <div className="w-full rounded-t"
                  style={{ height: `${Math.max(4, Math.round((d.count / maxDaily) * 80))}px`, background: "rgba(244,196,48,0.6)" }} />
              </div>
            ))}
          </div>
