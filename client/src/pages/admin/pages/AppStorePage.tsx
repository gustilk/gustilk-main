      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        {[
          ...
          <div key={s.store} data-testid={`store-${s.platform.toLowerCase()}`}
            className="rounded-2xl p-5 text-center" style={{ background: "rgba(255,255,255,0.10)", border: "1px solid rgba(255,255,255,0.07)" }}>
            <div className="text-3xl mb-2">{s.icon}</div>
            <div className="text-cream text-sm font-semibold">{s.store}</div>
            <div className="flex items-center justify-center gap-1 mt-2">
              {[1,2,3,4,5].map(i => <Star key={i} size={14} color="rgba(255,215,0,0.3)" fill="rgba(255,215,0,0.3)" />)}
            </div>
