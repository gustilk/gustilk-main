...
const CASTES = [
  { name: "Sheikh", description: "Religious leaders and their descendants. Highest spiritual authority in the Yezidi hierarchy.", color: "#FFD700", count: "—" },
...
export default function CastePage({ user }: { user: User }) {
  return (
    <div className="p-4 md:p-6 max-w-3xl mx-auto">
      ...
      <div className="space-y-3 mb-5">
        {CASTES.map(c => (
          <div key={c.name} data-testid={`caste-${c.name.toLowerCase()}`}
            ...
            style={{ background: "rgba(255,255,255,0.10)", border: `1px solid ${c.color}22` }}>
      ...
      {/* Compatibility rules */}
      <div className="rounded-2xl p-4" style={{ background: "rgba(255,255,255,0.10)", border: "1px solid rgba(255,215,0,0.15)" }}>
        <h3 className="text-cream text-sm font-semibold mb-3">Marriage Compatibility Rules</h3>
        <div className="space-y-2">
          {RULES.map((rule, i) => (
            <div key={i} className="flex items-start gap-2.5">
              <div className="w-1.5 h-1.5 rounded-full mt-1.5 flex-shrink-0" style={{ background: "#FFD700" }} />
              <span className="text-cream/60 text-xs">{rule}</span>
            </div>
          ))}
        </div>
        <div className="mt-3 pt-3 text-cream/30 text-xs" style={{ borderTop: "1px solid rgba(255,255,255,0.12)" }}>
