...
const CASTES = [
  { name: "Sheikh", description: "Religious leaders and their descendants. Highest spiritual authority in the Yezidi hierarchy.", color: "#F4C430", count: "—" },
...
export default function CastePage({ user }: { user: User }) {
  return (
    <div className="p-4 md:p-6 max-w-3xl mx-auto">
      ...
      <div className="space-y-3 mb-5">
        {CASTES.map(c => (
          <div key={c.name} data-testid={`caste-${c.name.toLowerCase()}`}
            ...
            style={{ background: "rgba(0,0,0,0.04)", border: `1px solid ${c.color}22` }}>
      ...
      {/* Compatibility rules */}
      <div className="rounded-2xl p-4" style={{ background: "rgba(0,0,0,0.04)", border: "1px solid rgba(244,196,48,0.15)" }}>
        <h3 className="text-cream text-sm font-semibold mb-3">Marriage Compatibility Rules</h3>
        <div className="space-y-2">
          {RULES.map((rule, i) => (
            <div key={i} className="flex items-start gap-2.5">
              <div className="w-1.5 h-1.5 rounded-full mt-1.5 flex-shrink-0" style={{ background: "#F4C430" }} />
              <span className="text-cream/60 text-xs">{rule}</span>
            </div>
          ))}
        </div>
        <div className="mt-3 pt-3 text-cream/30 text-xs" style={{ borderTop: "1px solid rgba(0,0,0,0.05)" }}>
