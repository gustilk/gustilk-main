...
export default function UserDetailPage({ user: adminUser, userId }: { user: User; userId: string }) {
  ...
  const resetPasskeysMutation = useMutation({
    ...
    onSuccess: () => { toast({ title: "Passkeys cleared — user can re-register on their new device" }); setPending(null); },
          ...
          {u.phone && (
            <button
              onClick={() => setPending({
                ...
                description: "Only do this after verifying their identity via selfie. They will need to re-register biometrics on their new device.",
                variant: "warning",
