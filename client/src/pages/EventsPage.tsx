...
const TYPE_COLORS: Record<string, { bg: string; text: string; border: string }> = {
  cultural: { bg: "rgba(255,215,0,0.12)", text: "#FFD700", border: "rgba(255,215,0,0.25)" },
  meetup: { bg: "rgba(255,215,0,0.12)", text: "#FFD700", border: "rgba(255,215,0,0.25)" },
  online: { bg: "rgba(123,63,160,0.15)", text: "#9b6bd4", border: "rgba(123,63,160,0.3)" },
};

const TYPE_BG: Record<string, string> = {
  cultural: "linear-gradient(135deg, #C0000F, #CC9900)",
  meetup: "linear-gradient(135deg, #C5000D, #A0000A)",
  online: "linear-gradient(135deg, #B5000A, #7A0007)",
};
    ...
    <div className="flex flex-col min-h-screen pb-24" style={{ background: "#E30613" }}>
        ...
        {TYPES.map(tab => (
          <button
            ...
            style={activeType === tab.id
              ...
              : { background: "rgba(255,255,255,0.12)", color: "rgba(255,255,255,0.45)", border: "1px solid rgba(255,215,0,0.15)" }
            }
          >
            {tab.label}
          </button>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center h-48 gap-3 text-center px-8">
          <CalendarDays size={36} color="rgba(255,215,0,0.4)" />
...
function EventFormScreen({ initial, isEditing, isPending, onBack, onSubmit }: {
  ...
  const dividerStyle = { height: "1px", background: "rgba(255,215,0,0.08)", margin: "0 16px" };

  const typeOptions: { value: FormData["type"]; label: string; emoji: string; color: string }[] = [
    { value: "meetup", label: "Meetup", emoji: "🤝", color: "#FFD700" },
    { value: "cultural", label: "Cultural", emoji: "🏛", color: "#FFD700" },
    { value: "online", label: "Online", emoji: "💻", color: "#9b6bd4" },
  ];

  return (
    <div className="fixed inset-0 z-[60] flex flex-col" style={{ background: "#E30613" }}>
      {/* Native-style nav bar */}
      <div
        className="flex items-center justify-between px-4 flex-shrink-0"
        style={{ paddingTop: "calc(env(safe-area-inset-top) + 48px)", paddingBottom: "12px", borderBottom: "1px solid rgba(255,215,0,0.12)" }}
      >
        <button
          onClick={onBack}
        ...
        <button
          ...
          style={{ color: isValid && !isPending ? "#FFD700" : "rgba(255,215,0,0.3)" }}
        >
      ...
      <div className="flex-1 overflow-y-auto px-4 pt-6" style={{ paddingBottom: "calc(env(safe-area-inset-bottom) + 40px)" }}>
          ...
          {typeOptions.map(opt => (
            <button
              ...
              style={form.type === opt.value
                ...
                : { background: "rgba(255,255,255,0.10)", border: "1.5px solid rgba(255,255,255,0.14)", color: "rgba(255,255,255,0.3)" }
              }
            >
        ...
        <button
          ...
          style={{ background: isValid ? "linear-gradient(135deg, #A0000A, #FFD700)" : "rgba(255,255,255,0.12)", color: "white" }}
        >
...
function EventCard({ event, user, onAttend, onOpen, onEdit, onDelete, isPending }: {
  ...
  return (
    <div
      ...
      style={{ border: "1px solid rgba(255,215,0,0.12)", background: "rgba(255,255,255,0.10)" }}
      ...
      <button onClick={onOpen} className="w-full text-left">
        <div className="h-32 relative flex items-center justify-center" style={{ background: bgGradient }}>
          ...
          <div className="absolute bottom-3 right-3 flex items-center gap-1">
            <Users size={11} color="rgba(255,255,255,0.6)" />
        ...
        <div className="p-4 pb-3">
          ...
          <div className="flex items-center gap-1.5 mb-1">
            <CalendarDays size={11} color="rgba(255,215,0,0.7)" />
            <span className="text-cream/50 text-xs">{dateLabel}</span>
          </div>
          <div className="flex items-center gap-1.5">
            <MapPin size={11} color="rgba(255,215,0,0.7)" />
      ...
      <div className="px-4 pb-4 flex items-center gap-2">
        <button
          ...
          style={event.isAttending
            ...
            : { background: "linear-gradient(135deg, #A0000A, #FFD700)", color: "white" }
          }
        >
        ...
        <button
          ...
          style={{ background: "rgba(255,255,255,0.12)", color: "rgba(255,255,255,0.4)" }}
        >
        ...
        {canManage && (
          <>
            <button
              ...
              className="p-2.5 rounded-xl transition-all"
              style={{ background: "rgba(255,255,255,0.12)", color: "rgba(255,215,0,0.5)" }}
            >
              <Edit2 size={14} />
            </button>
            {confirmDelete ? (
              ...
              <button
                ...
                className="p-2.5 rounded-xl transition-all"
                style={{ background: "rgba(255,255,255,0.12)", color: "rgba(239,68,68,0.4)" }}
              >
                <Trash2 size={14} />
              </button>
            )}
